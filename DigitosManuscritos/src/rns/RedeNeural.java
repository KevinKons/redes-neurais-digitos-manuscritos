...
public class RedeNeural {
    ...
    public double[][] aprender(double[][] entradasParaConstrucao, double[][] saidasParaConstrucao,
        ...
        for (int i = 0; i < entradasParaTeste.length; i++) {
            MLData casoEntrada = new BasicMLData(entradasParaTeste[i]);
            MLData saidaCaso = network.compute(casoEntrada);
            saidaDaRede[i] = saidaCaso.getData();
            System.out.println("Entrada caso" + i + ": " + casoEntrada + " --> Saída: " + saidaCaso);
            entradaSaida += "Entrada caso" + i + ": " + casoEntrada + " --> Saída: " + saidaCaso + "\n";
        }
